...
try
	{
		...
		{
			case "ED":
			$consulta = $_TABLATABLONEDITORIAL->Update(array_slice($_POST, 1) );
			break;
			case "RE":
			$consulta = $_TABLAREVISTAS->Update(array_slice($_POST, 1) );
			break;
			case "CON":
			$consulta = $_TABLACONFERENCIASORG->Update(array_slice($_POST, 1) );
			break;
		}
	...
	catch(Exception $e)
	{
		...
		if (!isset($_COOKIE["TEST"]))
		{
			...
			$_SESSION['error'] = 'ERROR MOD A'."=>".$errorRescrito[1];
			header("Location: ".$controladores[$identificadoresPrivados["Actividades"]]);
		}
	}
