...
Class AltaPublicacionesPrivada
{
...
?>
	<section id="content">
		<div class="container">
					  <div class="row">
				<div class="col-md-12">
					<form role="form" action="<?php echo $procesadores[$identificadoresPrivados["APublicaciones"]]?>" method="POST">
					<input type="hidden" name="TIPO" value="<?php echo $_POST["TIPO"]?>"/>
					<?php
					foreach($formularios[$identificadoresPrivados["APublicaciones"].$_POST["TIPO"]] as $campos)
					{
									...
									switch (explode(":",$campos[3])[0])
								  {
									  ...
									  $opciones2 = array();
										switch (explode(":",$campos[4])[0])
										  {
											  case "sql":
											  $opciones2 = TablaBD::ConsultaGenerica(explode(":",$campos[4])[1]);
											  for ($i = 0 ; $i < $opciones->num_rows;$i = $i +1)
											  {
												$dato = $opciones2->fetch_assoc();
									  ...
									  $opciones2 = array();
											switch (explode(":",$campos[4])[0])
											  {
												  case "sql":
												  $consulta = TablaBD::ConsultaGenerica(explode(":",$campos[4])[1]);
												  for ($i = 0 ; $i < $opciones->num_rows;$i = $i +1)
												  {
													  $dato = $opciones2->fetch_assoc();
													  $opciones2[] = $dato[array_keys($dato)[0]];
...
$princ_view = new AltaPublicacionesPrivada();
