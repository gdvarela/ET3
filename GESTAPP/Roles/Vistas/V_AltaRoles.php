...
<div id="contenido">
	<div id="barraLocalizacion">
		...
		<div id="histAct">
			> <?php echo $idioma['Nuevo Rol']?> 
		</div>
		<img onClick="window.history.back()" id="historialAtras" src="../../Imagenes/historyBack.png" />
	</div>
	<!-- A partir de aqui meteis vuestro codigo -->
	<form id="frmAltaRol" action='ProcesarAltaRol.php' method='POST' onSubmit="return validarForm()" >
		<div id="div_cuerpo" >
			<h1><?php echo $idioma['TIT ALT R'];?></h1>
			<?php
				if ($datosPrevios->ROL_nombre == "") 
					echo '<input type="hidden" id="validForm" name="validado" value="ROL_nombre" disabled />';
				else
			...
			<table width=100%>   				   
				...
				<tr>
					<td width=50%>
						<input type="text" size="20" name="ROL_nombre" onBlur="esVacio(this,'NombreVacio')" value="<?php echo $datosPrevios->ROL_nombre; ?>" />
						<span id="NombreVacio" class="error"><?php echo$idioma['DATOS VACIOS']; ?></span>
					</td>
					<td rowspan=2>
						<textarea  type="text" cols = "40" rows = "3" name="ROL_descripcion" ><?php echo $datosPrevios->ROL_descripcion; ?></textarea>
					</td>
				</tr>
				<tr>
					<td>
